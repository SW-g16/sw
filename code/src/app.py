...
def display():
    q = "select ?uri ?text where {?uri a hume:Law.?uri hume:stringified ?text.} limit 5";
    r = query_our_db(q)
    return render_template('display.html', result=r, store_endpoint=ENDPOINT + DB_NAME, website=WEBSITE)
...
def import_():
    ...
    return render_template('import.html', store_endpoint=ENDPOINT,db_ns=DB_NS,db_name=DB_NAME,website=WEBSITE)
...
def store():
    ...
    transaction_close_url = ENDPOINT + DB_NAME + "/transaction/commit/" + transaction_id
